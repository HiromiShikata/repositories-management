...
describe('create-pr.yml workflow', () => {
  ...
  test('Enable Auto Merge step uses bot token and conditional merge method so feature branch commits appear in contribution graph', () => {
    const stepStart = workflowContent.indexOf('- name: Enable Auto Merge for PR');
    const nextStep = workflowContent.indexOf('- name:', stepStart + 1);
    const stepBlock =
      nextStep === -1
        ? workflowContent.slice(stepStart)
        : workflowContent.slice(stepStart, nextStep);
    expect(stepBlock).toContain('steps.app-token.outputs.token');
    expect(stepBlock).not.toContain('secrets.GH_TOKEN');
    expect(stepBlock).toContain('steps.check_auto_merge.outputs.merge_method');
  });

  test('Check auto-merge capability step outputs merge method based on allow_merge_commit setting', () => {
    const stepStart = workflowContent.indexOf('- name: Check auto-merge capability');
    const nextStep = workflowContent.indexOf('- name:', stepStart + 1);
    const stepBlock =
      nextStep === -1
        ? workflowContent.slice(stepStart)
    ...
    expect(stepBlock).toContain('merge_method=MERGE');
    expect(stepBlock).toContain('merge_method=SQUASH');
  });
});
